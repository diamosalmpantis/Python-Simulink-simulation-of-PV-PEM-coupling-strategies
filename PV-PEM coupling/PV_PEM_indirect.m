...
% Real-world converter design parameters (will be scaled below for time compression).
Ts          = 1e-3;         % [s]   real switching period  (fsw = 1 kHz)
Tsc         = 1e-5;         % [s]   real Simscape solver step (placeholder — overridden)
sample_output_time = 1e-2;  % [s]   placeholder — overridden to Irr_stair_tsamp below
...
%
%  Root cause of incorrect results under time compression:
%    Irr_stair_tsamp = 60/36000 = 1.667e-3 s per weather minute.
%    With Ts = 1e-3 s (original), only ~1.7 switching cycles exist per
%    weather step.  The P&O MPPT algorithm needs >> 1 switching cycle to
%    evaluate dP/dV and update the duty cycle — it never converges.
%
%  Fix — scale Ts, L, C together so N_sw_per_step switching cycles
%  occur per weather step, keeping fn/fsw = constant:
%
%    Ts_sim   = Irr_stair_tsamp / N_sw_per_step
%    L_sim    = L_real × (Ts_sim / Ts_real)     → fn ∝ 1/√(LC) scales correctly
%    C_sim    = C_real × (Ts_sim / Ts_real)
%    Tsc_sim  = Ts_sim / 20                      → local solver << switching
%    Ki_sim   = Ki_real × (Ts_real / Ts_sim)     → preserve integral bandwidth
%    MaxStep  = Ts_sim / 5                        → ode15s resolves each cycle
%    sample_output_time = Irr_stair_tsamp         → 1 sample per weather step
%
%  Mathematical proof that fn/fsw stays constant:
%    fn_sim = 1/(2π√(L_sim × C_sim)) = 1/(2π × scale_Ts × √(L×C)) = fn_real/scale_Ts
%    fsw_sim = 1/Ts_sim = fsw_real / scale_Ts
%    → fn_sim / fsw_sim = fn_real / fsw_real  (ratio unchanged) ✓
% =========================================================================
N_sw_per_step = 2;                              % switching cycles per 1-min weather step
Ts_real       = Ts;                              % keep real-world reference
L_conv_real   = L_conv;
C_conv_real   = C_conv;
fn_real       = fn;

Ts       = Irr_stair_tsamp / N_sw_per_step;     % [s] scaled switching period
scale_Ts = Ts / Ts_real;                         % dimensionless ratio (< 1)

L_conv = L_conv_real * scale_Ts;                % [H] scaled — maintains fn/fsw
C_conv = C_conv_real * scale_Ts;                % [F] scaled — maintains fn/fsw
fn_sim = 1 / (2*pi*sqrt(L_conv * C_conv));      % [Hz] natural frequency (scaled)

Tsc                = Ts / 20;                   % [s] local solver step << switching
sample_output_time = Irr_stair_tsamp;           % [s] 1 output sample per weather step

% MPPT integral gain: scale so the continuous-time bandwidth is preserved.
% Original discrete Ki integrates at rate Ki × Ts per error unit.
% After scaling: Ki_sim × Ts_sim = Ki_real × Ts_real  → Ki_sim = Ki_real/scale_Ts
Ki_sim = 10.0 / scale_Ts;    % scaled integral gain
Kp_sim = 0.01;                % proportional gain is frequency-independent

fprintf('\n--- Converter parameter scaling for time compression ---\n');
fprintf('  scale_factor     : %d  |  N_sw_per_step : %d\n', scale_factor, N_sw_per_step);
fprintf('  Switching period : Ts_real=%.2e s → Ts_sim=%.2e s  (×%.1f)\n', ...
    Ts_real, Ts, 1/scale_Ts);
fprintf('  Filter L         : %.2e → %.2e H\n', L_conv_real, L_conv);
fprintf('  Filter C         : %.2e → %.2e F\n', C_conv_real, C_conv);
fprintf('  Natural freq fn  : %.1f → %.1f Hz  (fn/fsw = %.3f, unchanged)\n', ...
    fn_real, fn_sim, fn_sim * Ts);
fprintf('  MPPT Ki          : %.1f → %.2f  |  Kp = %.4f (unchanged)\n', ...
    10.0, Ki_sim, Kp_sim);
fprintf('  Tsc (local)      : %.2e s  |  sample_output_time : %.2e s\n\n', ...
    Tsc, sample_output_time);

% Push scaled values to base workspace (Simulink blocks read these by name)
...
case_model = 'D';
mdl_orig   = 'PV_PEM_converter_d';
mdl_fixed  = 'PV_PEM_converter_d_FIXED';
...
for i = 1:length(blocks)
    ...
    % Kp unchanged; Ki scaled to preserve integral bandwidth at new Ts.
    if contains(b_name, 'Control', 'IgnoreCase', true) || ...
       contains(b_name, 'MPPT_Control') || strcmp(b_type, 'Saturation')
        ...
        try set_param(b, 'I',  num2str(Ki_sim));   catch; end
    ...
    % Filter inductor and capacitor — set to scaled workspace variables.
    % Simscape inductor/capacitor blocks accept a parameter name string so
    % that the scaled L_conv / C_conv from the workspace takes effect.
...
set_param(mdl, 'MaxStep',        num2str(Ts / 5));   % resolve scaled switching cycle
...
% =========================================================================
filename = 'PV_PEM_indirect_results_week_new_size.xlsx';
 
% --- Derived quantities ---
% PV area: Amerisolar 320W has 60 M6 cells per panel (166×166 mm each).
% The model uses Ns_cell=45 cells per string. Because Ns_cell ≠ N_cells_per_panel,
% the physical area of one model string = Ns_cell × cell_area, NOT 1.689 m².
...
PV_area           = Ns_cell * Np_cell * cell_area;   % = 45×7×0.02815 = 8.87 m²
fprintf('PV area (cell-based): %.4f m²  |  STC η_PV check: %.1f %%\n', ...
        PV_area, Pmpp / (1000 * PV_area) * 100);
LHV_H2  = 119960;             % [J/g]
...
P_chem  = H2_inst_g_s .* LHV_H2;              % [W]
eta_PEM = P_chem ./ max(PEM_P, 1e-3) * 100;   % [%]
eta_PEM(Irr < 70) = NaN;

% Efficiency chain: STH = η_PV × η_converter × η_PEM / 10000
% For indirect coupling, η (PEM_P/PV_P) captures converter losses.
%   STH [%] = (η_PV/100) × (η_conv/100) × (η_PEM/100) × 100
active_day    = Irr > 70;
% Use instantaneous (unsmoothed) converter efficiency for the chain so the
% product telescopes: (PV_P/P_solar)*(PEM_P/PV_P)*(P_chem/PEM_P) = P_chem/P_solar.
% The smoothed `eta` used for plotting breaks this identity.
eta_conv_inst = PEM_P ./ max(PV_P, 1e-3) * 100;
eta_conv_inst(~active_day) = NaN;
STH_chain     = eta_PV .* eta_conv_inst .* eta_PEM / 1e4;   % [%]
fprintf('--- Efficiency chain check (G > 70 W/m2) ---\n');
fprintf('  mean eta_PV              : %.2f %%\n', mean(eta_PV(active_day),        'omitnan'));
fprintf('  mean eta_converter (plot): %.2f %%\n', mean(eta(active_day),           'omitnan'));
fprintf('  mean eta_converter (inst): %.2f %%\n', mean(eta_conv_inst(active_day), 'omitnan'));
fprintf('  mean eta_PEM             : %.2f %%\n', mean(eta_PEM(active_day),       'omitnan'));
