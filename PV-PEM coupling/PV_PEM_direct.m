...
if isfile('weather_profile_week.xlsx')
    ...
    % Annual mock: 7 days at 1-hour resolution
...
% =========================================================================
case_model = 'A';
mdl_orig   = 'PV_PEM_direct_a';
mdl_fixed  = 'PV_PEM_direct_a_FIXED';
...
% Rated operating point = load-line intersection at Vmax:
%   I_rated = (Vmax - Vint_stack) / R_total  ≈ 47.9 A  → P_max ≈ 1245 W
...
n_H2             = (N * PEM_I) / (2 * F_const) * eta_F;   % [mol/s]
...
% =========================================================================
filename        = 'PV_PEM_direct_results.xlsx';
...
% --- Derived quantities ---
% 1.689 m² is the panel area per STRING (one series column of Ns_cell cells).
% PV area: Amerisolar 320W has 60 M6 cells per panel (166×166 mm each).
% The model uses Ns_cell=45 cells per string (chosen so Vmpp=25.2 V fits
% within the PEM operating window 19.5–26 V for direct coupling).
% Because Ns_cell ≠ N_cells_per_panel, the physical area of one model
% string = Ns_cell × cell_area, NOT the full 1.689 m² panel area.
...
% H2_rate [g/h] / 3600 [s/h] × LHV_H2 [J/g] = chemical power [W]
% Divided by incident solar power [W] × 100 → percent.
STH = ((H2_rate/3600) .* LHV_H2) ./ max(PV_area .* Irr, 1e-3) * 100;

% PV solar-to-electric efficiency [%]  η_PV = P_elec / (G × A_PV)
...
% η_coupling ≈ eta (PEM_P_smooth / PV_P_smooth from Section 6).
% Small discrepancy expected because eta uses smoothed power traces.
active_day  = Irr > 70;
STH_chain   = eta_PV .* eta .* eta_PEM / 1e4;   % [%]
fprintf('--- Efficiency chain check (G > 70 W/m²) ---\n');
