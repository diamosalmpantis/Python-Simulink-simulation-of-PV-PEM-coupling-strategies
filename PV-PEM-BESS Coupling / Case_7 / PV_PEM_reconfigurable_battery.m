...
% title(tl1,'PEM Power and Irradiance – 8-Day Overview','FontSize',14,'FontWeight','bold');

ax1a = nexttile;
fill([t_c; flipud(t_c)], [Irr_c{1}; zeros(size(t_c))], ...
    clr_irr, 'FaceAlpha',0.35, 'EdgeColor','none');
ylabel('G  (W m^{-2})'); ylim([0 1300]);
text(0.01, 0.90, 'Solar irradiance', 'Units','normalized','FontSize',13,'Color',[0.55 0.48 0]);
...
% title(ax2,'Cumulative Hydrogen Production – 8-Day','FontSize',14,'FontWeight','bold');
for k = 1:NC
    lw_k = lw + 0.2;
...
legend(ax2,'Location','northwest','FontSize',13);
...
% title(ax3,'Battery State of Charge – 8-Day','FontSize',14,'FontWeight','bold');
for k = 1:NC
    lw_k = lw;
...
text(ax3, NUM_DAYS*24*0.995, 92, 'SOC_{max}=90%', ...
    'HorizontalAlignment','right','FontSize',12,'Color',[0.6 0.1 0.1]);
...
ylim(ax3,[0 100]);
legend(ax3,'Location','southoutside','FontSize',13,'NumColumns',3);
...
% title(tl4,'Efficiency Metrics – 8-Day Overview','FontSize',14,'FontWeight','bold');
...
for k = 1:NC
    ...
    title(NM_SHORT{k},'FontSize',13,'FontWeight','bold');
...
% title(tl6,'Power Balance – PV | PEM | Battery','FontSize',14,'FontWeight','bold');
...
for k = 1:NC
    ...
    ylabel(ax,'Power  (W)');
    title(ax, NM{k},'FontSize',13,'FontWeight','bold');
    legend(ax,'Location','northeast','FontSize',12,'NumColumns',2);
    xticks(ax,day_ticks); xticklabels(ax,day_labels); xlim(ax,x_lim);
    add_day_sep(ax, day_ticks);
end
xlabel('Time (hours)');
...
% Widened from 1920 so the four "Case N" tick labels fit each column without
% colliding (they previously overlapped into an unreadable cluster in the
% narrower panels). 'Visible','off' is required: an on-screen figure is
% clamped to the display width, which silently capped the exported width and
% made the extra width have no effect.
fig7 = figure('Position',[90 90 2400 800],'Visible','off');
...
bar_lbl = @(ax,vals,fmt,off) arrayfun(@(k) ...
    ...
    'HorizontalAlignment','center','VerticalAlignment','bottom','FontSize',13,'FontWeight','bold'), 1:numel(vals));
...
ylabel(ax7a,'8-day H_2 yield  (g)');
% Headroom factor raised from 1.22 so the "north" legend clears the value
% label above the tallest bar (they collided at 1.22).
h2_ymax = max(kpi.H2_tot)*1.42; if ~isfinite(h2_ymax)||h2_ymax<=0, h2_ymax=10; end
ylim(ax7a,[0 h2_ymax]);
title(ax7a,'H_2 Produced (PV + Batt)','FontWeight','bold');
grid(ax7a,'on'); box(ax7a,'on');
legend(ax7a,{'From PV','From Battery'},'Location','north','FontSize',12);
for k = 1:NC
...
bar_lbl(ax7c, kpi.mean_sth, '%.1f%%', sth_mx*0.025);   % 1 dp: 2 dp labels collided
...
ylim(ax7d,[0 tth_mx]);
title(ax7d,'Total-to-H_2 Efficiency (TTH)','FontWeight','bold');
grid(ax7d,'on'); box(ax7d,'on');
bar_lbl(ax7d, kpi.mean_tth, '%.1f%%', tth_mx*0.025);   % 1 dp: 2 dp labels collided
...
ylabel(ax7e,'Coupling factor'); ylim(ax7e,[0 1.62]);   % room for staggered labels
% Slightly wider than the default [0.5 NC+0.5] so the outermost value labels,
% which sit at k+-0.17, are not cut off by the axes boundary.
xlim(ax7e,[0.42 NC+0.62]);
...
% The two bars of a pair sit only 0.34 apart in x, so two 5-character labels
% placed at the same height touch. They are therefore staggered vertically:
% the C_PV label sits just above the taller bar of the pair and the C_sys
% label one step higher, which keeps both fully legible.
for k = 1:NC
    base = max(kpi.C_PV(k), kpi.C_sys(k)) + 0.03;
    text(ax7e,k-0.17, base,       sprintf('%.3f',kpi.C_PV(k)), ...
        'HorizontalAlignment','center','VerticalAlignment','bottom', ...
        'FontSize',12,'FontWeight','bold','Clipping','off');
    text(ax7e,k+0.17, base+0.095, sprintf('%.3f',kpi.C_sys(k)), ...
        'HorizontalAlignment','center','VerticalAlignment','bottom', ...
        'FontSize',12,'FontWeight','bold','Clipping','off');
end
...
for k = 1:NC
    if isfinite(kpi.eta_batt(k)) && kpi.eta_batt(k) > 0
        ...
        text(ax7g,k,3,'n/a','HorizontalAlignment','center','FontSize',12,'Color',[0.5 0.5 0.5]);
    end
end
...
legend(ax7h,{'Solar-sourced','Battery-sourced'},'Location','north','FontSize',12);
for k = 1:NC
    if kpi.H2_PV_pct(k) > 5
        text(ax7h,k,kpi.H2_PV_pct(k)/2, sprintf('%.0f%%',kpi.H2_PV_pct(k)), ...
            'HorizontalAlignment','center','FontSize',12,'Color','w','FontWeight','bold');
    end
...
% Pin the tick-label font across all eight KPI panels, then draw the category
% labels manually. MATLAB's tick engine horizontally compresses tick labels
% when a panel is too narrow for them, which rendered "Case 5" as an
% unreadable "Ca5" in the right-hand column regardless of figure size or font.
% Blanking XTickLabel and placing text objects instead bypasses that engine,
% so every panel shows the full "Case N" at the same size.
ax7_all = [ax7a ax7b ax7c ax7d ax7e ax7f ax7g ax7h];
set(ax7_all,'FontSize',12);
for a = ax7_all
    set(a,'XTickLabel',[]);
    yl_a = ylim(a);
    for k = 1:NC
        text(a, k, yl_a(1)-0.022*diff(yl_a), NM_BAR{k}, ...
...
% title(tl10,'Battery System Characterisation – 8-Day','FontSize',14,'FontWeight','bold');
...
legend(ax10a,{'Charged (Wh)','Discharged (Wh)'},'Location','north','FontSize',13);
...
text(ax10b, NC+0.42, 91.5, 'SOC_{max}=90%', 'FontSize',12, 'Color',[0.6 0.1 0.1]);
text(ax10b, NC+0.42, 17.5, 'SOC_{min}=20%', 'FontSize',12, 'Color',[0.6 0.1 0.1]);
hold(ax10b,'off');
...
% title(tl8,'H_2 Production Rate vs Irradiance Profile','FontSize',14,'FontWeight','bold');
...
% title(tl9,'Hydrogen Production Attribution – PV vs Battery','FontSize',14,'FontWeight','bold');
...
legend(ax9a, {'H_2 from PV','H_2 from Battery'}, 'Location','northeast','FontSize',13);
for k = 1:NC
...
legend(ax9b,'Location','northwest','FontSize',12,'NumColumns',2);
