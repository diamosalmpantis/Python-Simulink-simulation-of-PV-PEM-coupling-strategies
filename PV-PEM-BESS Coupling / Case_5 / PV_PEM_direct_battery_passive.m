...
% THE "REALISTIC CLOUDY DAY" IRRADIANCE PROFILE
% A natural bell curve interrupted by deep, realistic cloud covers.
Irr_hours = [0, 5.5, 6.5, 7.2, 7.8, 8.5, 9.2, 9.8, 10.5, 11.2, 12.0, 12.8, 13.5, 14.3, 15.0, 15.8, 16.6, 17.5, 18.5, 24];
Irr_time  = Irr_hours / 10; 
Irr_val   = [0,   0, 150, 380, 120, 520, 750, 250,  880,  300, 1050,  350,  920,  280,  700,  180,  400,  120,    0,  0];

% --- STAIR BLOCK ADAPTATION ---
Irr_stair_tsamp = 0.01; 
...
% =========================================================================
V_batt_nom  = 22.6; %12;            % [V]   nominal voltage
Q_batt      = 0.1;%2; %0.1;                % [Ah]  nominal capacity
Q_thesis    = 1000;             % Ah (Visual Scaling)
scaling = false;  %true if scaling should be done for the SOC plots
SOC_init_per    = 60;               % [%]   initial state of charge
SOC_init    = SOC_init_per/100*Q_batt;  % [Ah]   initial state of charge in Ah

R_batt_int  = 0.05;             % [Ω]   internal resistance (ESR)

SOC_max = 90;                   % [%]   charge cutoff
SOC_min = 20;                   % [%]   discharge cutoff
% Hysteresis bands
...
% =========================================================================
case_model = 'C';
mdl_orig  = 'PV_PEM_direct_battery_passive';
mdl_fixed = 'PV_PEM_direct_battery_passive_FIXED';
...
try
    ...
    if Q_batt <300 
        if scaling
            ...
            Batt_SOC_Math = SOC_init - (cumtrapz(real_t, Batt_I) / Q_coulombs) * 100;
            Batt_SOC = max(0, min(100, Batt_SOC_Math)); % Clamp strictly between 0% and 100%
...
if has_batt
    ...
    Total_In_P  = PV_P_smooth + max(Batt_P_smooth, 0);
    Total_Out_P = PEM_P_smooth + abs(min(Batt_P_smooth, 0));
else
...
if has_batt
    % Only count positive power (discharge) for the battery contribution
...
if has_batt
    %plot(t_plot,Batt_I,'Color',clr_batt,'LineWidth',lw,'LineStyle','--','DisplayName','|I_{batt}|');
    plot(t_plot,abs(Batt_I),'Color',clr_batt,'LineWidth',lw,'DisplayName','|I_{batt}|');
end
...
drawnow; exportgraphics(fig4, sprintf('Fig4_IV_Trajectory_Ns%d_Np%d.png', Ns_cell, Np_cell), 'Resolution', 300);
...
fig6 = figure('Name','Hourly Performance Summary','Color','w', ...
...
drawnow;
exportgraphics(fig6, sprintf('Fig6_DailySummary_Ns%d_Np%d_case_%s.png', Ns_cell, Np_cell, case_model), 'Resolution', 300);
% =========================================================================
%  SUMMARY TABLE
% =========================================================================
...
filename = 'PV_PEM_direct_battery_no_con_results.xlsx';
